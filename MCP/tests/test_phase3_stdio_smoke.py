...
async def _run_stdio_smoke(db_path: Path) -> str:
    ...
    async with stdio_client(parameters, errlog=errlog) as (read, write):
        async with ClientSession(read, write) as session:
            ...
            chapter = _data(await session.call_tool("chapter_create", {"title": "章"}))
            previous = _data(
            ...
            current_reveal = _data(
                await session.call_tool(
                    "information_create",
                    {"statement": "CURRENT_REVEAL_SENTINEL", "truth_status": "true"},
                )
            )
            _data(
                await session.call_tool(
            ...
            context = _data(
                await session.call_tool("episode_context", {"episode_id": target["id"]})
            )
            reveal_ids = {
