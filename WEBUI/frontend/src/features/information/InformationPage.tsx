...
function InformationEditor({ projectId, informationItemId, initial, onDirtyChange }: { projectId: string; informationItemId: number; initial: InformationItemRecord; onDirtyChange: (dirty: boolean) => void }) {
  ...
  return <>
    ...
    <CanonStatusControl projectId={projectId} entityType="information_item" record={baseline} dirty={dirty} onStatusChanged={async () => { const fetched = await fetchInformationItem(projectId, informationItemId); queryClient.setQueryData(projectQueryKeys.informationItem(projectId, informationItemId), fetched); setBaseline(fetched); setValues(toInformationForm(fetched)); }} readCurrent={() => fetchInformationItem(projectId, informationItemId)} />
